...
public class CustomCrashHandler implements Thread.UncaughtExceptionHandler{
    ...
    public void uncaughtException(Thread thread, Throwable ex) {
        ...
        // 提示用户程序即将退出
        showToast(mContext,
                "很抱歉，" + mContext.getResources().getString(R.string.app_name)
                        + "遭遇异常，即将退出！");
        try {
            Thread.sleep(2000);
     ...
     * @return
     */
    public static String saveInfoToSD(Context context, Throwable ex) {
        return saveInfoToSD(context, ex, true);
    }
     ...
     * @return
     */
    private static String saveInfoToSD(Context context, Throwable ex, boolean isError) {
        ...
        if (Environment.getExternalStorageState().equals(
            ...
            try {
                fileName = dir.toString() + File.separator
                        + parseTime(System.currentTimeMillis()) + (isError ? "" : "-Exception") + ".log";
     ...
     * @return
     */
    private static String parseTime(long milliseconds) {
